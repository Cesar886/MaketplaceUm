...
void main() {
  group('links del sitio', () {
    test('extrae el id de un link compartido', () {
      expect(idDe('https://marketplace-um.me/producto/p_123'), 'p_123');
    });

    test('acepta el subdominio www, que un link pegado a mano puede traer', () {
      expect(idDe('https://www.marketplace-um.me/producto/p_123'), 'p_123');
    });

    test('ignora query y fragmento, que agregan al reenviar por redes', () {
      expect(
        idDe('https://marketplace-um.me/producto/p_123?utm_source=wa#foto'),
        'p_123',
      );
    });

    test('decodifica un id escapado en la URL', () {
      expect(idDe('https://marketplace-um.me/producto/p%20123'), 'p 123');
    });

    test('acepta http, porque un link viejo puede no traer TLS', () {
      expect(idDe('http://marketplace-um.me/producto/p_123'), 'p_123');
    });
  });
  ...
  group('links que NO son de una publicación', () {
    test('rechaza otro dominio aunque la ruta calce', () {
      ...
      expect(idDe('https://marketplace-um.me.malo.com/producto/p_123'), isNull);
      expect(idDe('https://otrositio.com/producto/p_123'), isNull);
    });

    test('rechaza otra ruta del mismo dominio', () {
      expect(idDe('https://marketplace-um.me/'), isNull);
      expect(idDe('https://marketplace-um.me/blog/p_123'), isNull);
    });

    test('rechaza la ruta sin id', () {
      expect(idDe('https://marketplace-um.me/producto'), isNull);
      expect(idDe('https://marketplace-um.me/producto/'), isNull);
    });

    test('rechaza un id vacío o solo espacios', () {
      expect(idDe('https://marketplace-um.me/producto/%20'), isNull);
    });

    test('rechaza segmentos de más, que no son un id', () {
      expect(idDe('https://marketplace-um.me/producto/p_123/editar'), isNull);
    });

 ...
 */
async function getSellerAccess(database, userId, {
  nowMs = Date.now()
} = {}) {
  ...
  let row = await database.prepare(`SELECT id, admin_status, admin_status_reason, admin_status_until,
            auth_invalid_before
       FROM sellers
      WHERE id = ?`).get(userId);
  if (!row) {
  ...
  if (status === ACCOUNT_STATUSES.SUSPENDED) {
    ...
    if (untilMs !== null && untilMs <= nowMs) {
      ...
      const updated = await database.prepare(`UPDATE sellers
            SET admin_status = 'active',
                admin_status_reason = NULL,
                admin_status_until = NULL
          WHERE id = ?
            AND admin_status = 'suspended'
            AND admin_status_until IS ?`).run(userId, row.admin_status_until);
      if (updated.changes === 1) {
        ...
        row = await database.prepare(`SELECT id, admin_status, admin_status_reason, admin_status_until,
                  auth_invalid_before
             FROM sellers WHERE id = ?`).get(userId);
  ...
  if (status === ACCOUNT_STATUSES.BANNED) {
    return denied('ACCOUNT_BANNED', 'Esta cuenta fue inhabilitada por el equipo de Marketplace UM.', {
      reason: row.admin_status_reason || null
    });
  }
  if (status === ACCOUNT_STATUSES.SUSPENDED) {
    return denied('ACCOUNT_SUSPENDED', 'Esta cuenta esta suspendida temporalmente.', {
      reason: row.admin_status_reason || null,
...
async function getSellerTokenAccess(database, decoded, options) {
  const access = await getSellerAccess(database, decoded && decoded.sub, options);
  if (!access.allowed) return access;
  const preciseIssuedAt = Number(decoded && decoded.auth_time_ms);
  const issuedAtMs = Number.isSafeInteger(preciseIssuedAt) && preciseIssuedAt > 0 ? preciseIssuedAt : Number.isFinite(Number(decoded && decoded.iat)) ? Number(decoded.iat) * 1000 : 0;
 ...
 */
async function invalidateSellerSessions(database, userId, {
  nowMs = Date.now()
} = {}) {
  const cutoff = Math.max(1, Math.trunc(nowMs) + 1);
  return await database.transaction(async () => {
    const result = await database.prepare(`UPDATE sellers
          SET auth_invalid_before = MAX(COALESCE(auth_invalid_before, 0), ?)
        WHERE id = ?`).run(cutoff, userId);
    if (result.changes !== 1) return false;
    await database.prepare(`UPDATE refresh_sessions
...
function sendSellerAccessError(res, access) {
  const forbidden = access && (access.code === 'ACCOUNT_BANNED' || access.code === 'ACCOUNT_SUSPENDED' || access.code === 'ACCOUNT_DISABLED');
  const payload = {

...
function assertProductionSecurityConfig() {
  ...
  const secretNames = ['JWT_SECRET', 'ADMIN_JWT_SECRET', 'ADMIN_TOTP_ENCRYPTION_KEY', 'ADMIN_BFF_SHARED_SECRET'];
  ...
  if (adminOrigin.protocol !== 'https:' || adminOrigin.origin !== String(process.env.ADMIN_PANEL_ORIGIN).replace(/\/$/, '')) {
    throw new Error('[boot] ADMIN_PANEL_ORIGIN debe ser un origen HTTPS exacto.');
  }
}
...
const routes = [require('./routes/categories'), require('./routes/products'), require('./routes/sellers'), require('./routes/cart'), require('./routes/listings'), require('./routes/highlightPlans'), require('./routes/notifications'), require('./routes/chat'), require('./routes/wanted'), require('./routes/feed'), require('./routes/verificacion'), require('./routes/admin'),
...
require('./routes/public'), require('./routes/comments'), require('./routes/questions'), require('./routes/search'), require('./routes/clientErrors'), require('./routes/privacy'), require('./routes/reports'), require('./routes/secreto'),
...
// lleva rato fallando en silencio con 401 en producción.
{
  ...
  console.log(`[boot] Pagos (Mercado Pago): ${resumen.configurado ? 'OK' : 'SIN CONFIGURAR'}` + (resumen.configurado ? '' : ` — faltan: ${resumen.faltantes.join(', ')}`));
}
const app = express();
configureProxy(app);
...
app.use('/api/admin', cors({
  ...
  allowedHeaders: ['Authorization', 'Content-Type', 'X-Admin-Client-IP', 'X-Admin-Client-Time', 'X-Admin-Client-Signature'],
  credentials: false,
  maxAge: 600
}));
app.use(cors({
...
app.use('/uploads', async (req, res, next) => {
  ...
  const esEvidenciaPrivada = await db.getDb().prepare('SELECT 1 FROM verification_documents WHERE file_url = ? LIMIT 1').get(fileUrl);
...
const presencia = crearRegistroPresencia({
  guardarUltimaActividad: (userId, iso) => {
    void db.setUltimaActividad(userId, iso).catch(error =>
      console.error('[presence] no se guardó last_active:', error.message));
  },
  emitirCambio: ({
    userId,
    online,
    ...
    void db.getPresencia(userId).then(state => {
      // La preferencia puede haber cambiado mientras había suscriptores.
      if (!state.comparteEstado) return;
      io.to(`presence:${userId}`).emit('presence:update', { userId, online, lastActive });
    }).catch(error =>
      console.error('[presence] no se pudo emitir el cambio:', error.message));
  }
});
...
io.on('connection', socket => {
  ...
  // vivir horas.
  const validRealtimeId = value => typeof value === 'string' && value.length > 0 && value.length <= 180 && !/[\u0000-\u001f\u007f]/.test(value);
  const onAsync = (event, handler) => socket.on(event, (...args) => {
    void Promise.resolve(handler(...args)).catch(error => {
      console.error(`[socket] ${event} falló:`, error.message);
      socket.emit('server:error', { error: 'No se pudo completar la operación.' });
    });
  });
  onAsync('join:conversation', async conversationId => {
    if (!validRealtimeId(conversationId)) return;
    if (!(await db.esParticipanteDeConversacion(conversationId, socket.data.userId))) {
      console.warn(`[socket] ${socket.data.userId} intentó unirse a conv:${conversationId} sin participar en ella`);
      return;
    }
  ...
  onAsync('typing:start', async (payload = {}) => {
    const {
      conversationId
    } = payload;
    if (!validRealtimeId(conversationId) || !(await db.esParticipanteDeConversacion(conversationId, socket.data.userId))) return;
    const conv = await db.getDb().prepare('SELECT buyer_id, seller_id FROM conversations WHERE id = ?').get(conversationId);
    const otherId = conv?.buyer_id === socket.data.userId ? conv?.seller_id : conv?.buyer_id;
    if (await db.areUsersBlocked(socket.data.userId, otherId)) return;
    socket.to(`conv:${conversationId}`).emit('typing:start', {
      userId: socket.data.userId
    });
  });
  onAsync('typing:stop', async (payload = {}) => {
    const {
      conversationId
    } = payload;
    if (!validRealtimeId(conversationId) || !(await db.esParticipanteDeConversacion(conversationId, socket.data.userId))) return;
...
const {
  calcularIniciales: computeInitials
...
app.post('/api/auth/login', ...authLimiters, async (req, res) => {
  const {
    email,
    password,
    deviceId
  ...
  const row = await db.getDb().prepare('SELECT * FROM sellers WHERE email = ? COLLATE NOCASE').get(email.trim());
  ...
  if (!valid) {
    ...
    if (row) {
      ...
      if (attempts >= LOGIN_MAX_ATTEMPTS) {
        ...
        await db.getDb().prepare('UPDATE sellers SET failed_login_attempts = 0, locked_until = ? WHERE id = ?').run(lockedUntil, row.id);
      } else {
        await db.getDb().prepare('UPDATE sellers SET failed_login_attempts = ? WHERE id = ?').run(attempts, row.id);
      }
    }
  ...
  const accountAccess = await getSellerAccess(db.getDb(), row.id);
  ...
  if (row.failed_login_attempts || row.locked_until) {
    await db.getDb().prepare('UPDATE sellers SET failed_login_attempts = 0, locked_until = NULL WHERE id = ?').run(row.id);
  }
  if (deviceId) await db.linkDeviceToUser(deviceId, row.id);
  const session = await generateSession(row.id);
  res.json({
    ...session,
    seller: {
      id: row.id,
...
// y devuelve un JWT.
app.post('/api/auth/register', ...authLimiters, async (req, res) => {
  const {
    name,
    email,
    phone,
  ...
  if (emailError) return res.status(400).json({
    error: emailError
  });
  ...
  const existingRow = await db.getDb().prepare('SELECT * FROM sellers WHERE email = ? COLLATE NOCASE').get(email.trim());
  if (existingRow) {
    ...
    const accountAccess = await getSellerAccess(db.getDb(), existingRow.id);
    if (!accountAccess.allowed) return sendSellerAccessError(res, accountAccess);
    if (!existingRow.password_hash) {
      ...
      await updateSellerField(existingRow.id, 'password_hash', hash);
    }
    // Ya registrado: actualizar teléfono si cambió
    if (phone && phone.trim() !== existingRow.phone) {
      await updateSellerField(existingRow.id, 'phone', phone.trim());
    ...
    if (deviceId) await db.linkDeviceToUser(deviceId, existingRow.id);
    const session = await generateSession(existingRow.id);
    return res.json({
      ...session,
      seller: {
        id: existingRow.id,
  ...
  if (deviceId) await db.linkDeviceToUser(deviceId, newSeller.id);
  const session = await generateSession(newSeller.id);
  res.status(201).json({
    ...session,
    seller: {
      id: newSeller.id,
...
app.post('/api/auth/anon', createAnonymousSessionLimiter(), (req, res) => {
  if (typeof req.body?.deviceId !== 'string' || !/^[A-Za-z0-9._:-]{8,180}$/.test(req.body.deviceId)) {
    return res.status(400).json({
      error: 'Identificador de instalación inválido.'
    });
  }
  const {
    token,
...
app.post('/api/auth/refresh', async (req, res) => {
  const renewed = await refreshSession(req.body?.refreshToken);
  if (!renewed) {
    return res.status(401).json({
...
app.post('/api/auth/refresh/bootstrap', requireAuth, async (req, res) => {
  res.json(await generateSession(req.user.id));
});
app.post('/api/auth/logout', requireAuth, async (req, res) => {
  ...
  await revokeRefreshToken(req.body?.refreshToken, req.user.id);
  res.status(204).end();
});

// Health check
app.get('/api/health', async (_req, res) => {
  await db.getDb().prepare('SELECT 1').get();
  res.json({
    status: 'ok',
...
app.use((error, _req, res, _next) => {
  const status = error.status === 403 ? 403 : error.type === 'entity.too.large' ? 413 : error instanceof SyntaxError && error.status === 400 ? 400 : 500;
  if (status === 500) console.error('[api] Error no controlado:', error);
  const message = status === 403 ? 'Origen no permitido.' : status === 413 ? 'La solicitud supera el limite permitido.' : status === 400 ? 'JSON invalido.' : 'Error interno del servidor.';
...
async function start() {
  ...
  await db.limpiarInteraccionesAntiguas();
  await require('./routes/verificacion').reconciliarVerificacionesPendientesPorMercadoPago();

  setInterval(() => {
    void db.limpiarInteraccionesAntiguas().catch(error =>
      console.error('[db] la limpieza periódica falló:', error.message));
  }, 24 * 60 * 60 * 1000).unref();
  setInterval(() => {
    void ejecutarJobRetargeting().catch(error =>
      console.error('[retargeting] el job periódico falló:', error.message));
  }, RETARGETING_INTERVALO_MS).unref();
...
async function shutdown(signal) {
  ...
  shuttingDown = true;
  console.log(`[shutdown] ${signal}: cerrando HTTP y PostgreSQL`);
...
for (const signal of ['SIGTERM', 'SIGINT']) {
  process.once(signal, () => {
    void shutdown(signal).then(() => process.exit(0)).catch(error => {
      console.error('[shutdown] cierre incompleto:', error.message);
      process.exit(1);
    });
  });
}
...
void start().catch(error => {
  console.error('[boot] no se pudo iniciar Marketplace UM:', error.message);
  void Promise.resolve(db.closeDatabase?.()).finally(() => process.exit(1));
});

...
function crearRutasVerificacion({
  ...
  const obtenerSeller = async id => await getDb().prepare('SELECT id, verified, tipo_cuenta FROM sellers WHERE id = ?').get(id);
  const obtenerVerificacion = async usuarioId => await getDb().prepare('SELECT * FROM verificaciones WHERE usuario_id = ?').get(usuarioId);

  /** Crea la fila de verificación si aún no existe y la devuelve. */
  async function asegurarVerificacion(usuarioId, tipoCuenta) {
    const existente = await obtenerVerificacion(usuarioId);
    if (existente) return existente;
    await getDb().prepare(`INSERT INTO verificaciones (usuario_id, tipo_cuenta, estado, creado_en)
         VALUES (?, ?, 'pendiente', ?)`).run(usuarioId, tipoCuenta, ahora());
    return await obtenerVerificacion(usuarioId);
  }
   ...
   */
  async function marcarVerificado(usuarioId, camposExtra = {}, camposSellers = {}) {
  ...
  function exigirTipo(tipoEsperado) {
    return async (req, res, next) => {
      const seller = await obtenerSeller(req.user.id);
      if (!seller) {
  ...
  const minutosRestantes = inicioVentana => Math.max(1, Math.ceil((new Date(inicioVentana).getTime() + VENTANA_ENVIO_MINUTOS * 60 * 1000 - Date.now()) / 60000));

  /**
   * @returns {null|{ minutos: number }} null si puede enviar.
   */
  async function limiteAlcanzado(verificacion, columnaDestino, destino) {
    const inicioVentanaValida = new Date(Date.now() - VENTANA_ENVIO_MINUTOS * 60 * 1000).toISOString();

    // Límite por usuario.
    if (verificacion.ventana_envio_inicio && verificacion.ventana_envio_inicio > inicioVentanaValida && verificacion.intentos_envio >= MAX_ENVIOS) {
      return {
        minutos: minutosRestantes(verificacion.ventana_envio_inicio)
      };
    }

    // Límite por destino: suma los envíos que otras cuentas hicieron al mismo
    // correo/teléfono dentro de la ventana.
    const otras = await getDb().prepare(`SELECT intentos_envio, ventana_envio_inicio FROM verificaciones
         WHERE ${columnaDestino} = ? AND usuario_id != ?
           AND ventana_envio_inicio > ?`).all(destino, verificacion.usuario_id, inicioVentanaValida);
    for (const fila of otras) {
      if (fila.intentos_envio >= MAX_ENVIOS) {
        return {
          minutos: minutosRestantes(fila.ventana_envio_inicio)
        };
      }
    }
    return null;
  ...
  async function registrarEnvio(verificacion) {
    const ventanaVigente = verificacion.ventana_envio_inicio && new Date(verificacion.ventana_envio_inicio).getTime() > Date.now() - VENTANA_ENVIO_MINUTOS * 60 * 1000;
    await getDb().prepare(`UPDATE verificaciones
         SET intentos_envio = ?, ventana_envio_inicio = ?
         WHERE usuario_id = ?`).run(ventanaVigente ? verificacion.intentos_envio + 1 : 1, ventanaVigente ? verificacion.ventana_envio_inicio : ahora(), verificacion.usuario_id);
  }
  ...
  async function solicitarOtp(res, {
    ...
    const limite = await limiteAlcanzado(verificacion, columnaDestino, destino);
    if (limite) {
    ...
    try {
      ...
      console.error(`[verificacion] Envío fallido (code=${err.code}): ${err.message}`);
    ...
    await registrarEnvio(verificacion);
    return res.json({
      enviado: true,
   ...
   */
  async function marcarIdentidadConfirmada(usuarioId, columnaCodigo, columnaExpira) {
    await getDb().prepare(`UPDATE verificaciones SET
   ...
   */
  async function completarSiPuedeCobrar(res, verificacion, camposSellers) {
    const falta = await primerFaltante(verificacion.usuario_id);
    if (falta) {
      ...
      return res.json({
        ...
        requisitos: await requisitosDeVerificacion(verificacion.usuario_id),
        tipo_cuenta: verificacion.tipo_cuenta
      });
    }
    await marcarVerificado(verificacion.usuario_id, {}, camposSellers);
    return res.json({
      verificado: true,
      estado: 'verificado',
      tipo_cuenta: verificacion.tipo_cuenta
  ...
  async function confirmarOtp(res, {
    verificacion,
    columnaCodigo,
    columnaExpira,
    camposSellers = {}
    ...
    if (verificacion.identidad_confirmada_en) {
      return await completarSiPuedeCobrar(res, verificacion, camposSellers);
    }
    const resultado = verificarCodigo(typeof res.req.body.codigo_otp === 'string' ? res.req.body.codigo_otp.trim() : '', verificacion[columnaCodigo], verificacion[columnaExpira]);
    if (resultado.ok) {
      await marcarIdentidadConfirmada(verificacion.usuario_id, columnaCodigo, columnaExpira);
      return await completarSiPuedeCobrar(res, verificacion, camposSellers);
    }
    ...
    if (resultado.razon === 'expirado') {
      await getDb().prepare(`UPDATE verificaciones SET ${columnaCodigo} = NULL, ${columnaExpira} = NULL
           WHERE usuario_id = ?`).run(verificacion.usuario_id);
      return res.status(400).json({
    ...
    await getDb().prepare(`UPDATE verificaciones SET
         ...
         WHERE usuario_id = ?`).run(intentos, verificacion.usuario_id);
    return res.status(400).json({
      error: agotados ? 'Demasiados intentos fallidos. Solicita un código nuevo.' : 'El código no es correcto.',
  ...
  router.post('/estudiante/solicitar', requireAuth, exigirTipo('estudiante'), async (req, res) => {
    ...
    const yaUsado = await getDb().prepare(`SELECT correo_institucional FROM verificaciones
           WHERE (correo_institucional = ? OR matricula = ?)
             AND estado = 'verificado' AND usuario_id != ?`).get(correo, matricula, req.user.id);
    if (yaUsado) {
      return res.status(409).json({
        error: yaUsado.correo_institucional === correo ? 'Ese correo institucional ya está registrado en otra cuenta.' : 'Esa matrícula ya está registrada en otra cuenta.',
        campo: 'correo_institucional'
      });
    }
    const verificacion = await asegurarVerificacion(req.user.id, 'estudiante');
    return await solicitarOtp(res, {
      verificacion,
      destino: correo,
  ...
  router.post('/estudiante/confirmar', requireAuth, exigirTipo('estudiante'), async (req, res) => {
    const verificacion = await asegurarVerificacion(req.user.id, 'estudiante');
    return await confirmarOtp(res, {
      verificacion,
      columnaCodigo: 'codigo_otp_email',
      columnaExpira: 'codigo_otp_email_expira',
  ...
  router.post('/negocio/documentos', requireAuth, exigirTipo('negocio'), (req, res) => {
    documentUpload.single('file')(req, res, async error => {
      ...
      if (docType !== 'additional_evidence' && path.extname(req.file.originalname).toLowerCase() === '.pdf') {
        descartarArchivo();
        return res.status(400).json({
      ...
      if (docType === 'additional_evidence') {
        const existente = await db.prepare('SELECT file_url FROM verification_documents ' + 'WHERE usuario_id = ? AND doc_type = ? AND content_hash = ? ' + 'ORDER BY id DESC LIMIT 1').get(req.user.id, docType, contentHash);
        if (existente) {
          descartarArchivo();
        ...
        await db.prepare('DELETE FROM verification_documents ' + 'WHERE usuario_id = ? AND doc_type = ?').run(req.user.id, docType);
      }
      await db.prepare('INSERT INTO verification_documents ' + '(usuario_id, doc_type, file_url, original_name, mime_type, ' + 'uploaded_at, content_hash) VALUES (?, ?, ?, ?, ?, ?, ?)').run(req.user.id, docType, archivo, path.basename(req.file.originalname), req.file.mimetype || 'application/octet-stream', ahora(), contentHash);
  ...
  router.post('/negocio/solicitar-manual', requireAuth, exigirTipo('negocio'), async (req, res) => {
    const db = getDb();
    const perfil = await db.prepare('SELECT name, businessCategory FROM sellers WHERE id = ?').get(req.user.id);
    ...
    const requisitoFaltante = await primerFaltante(req.user.id);
    ...
    const documentos = await db.prepare('SELECT doc_type FROM verification_documents WHERE usuario_id = ?').all(req.user.id);
    ...
    await asegurarVerificacion(req.user.id, 'negocio');
    await db.transaction(async () => {
      await db.prepare(`UPDATE verificaciones SET
           estado = 'pendiente',
           creado_en = ?,
           nombre_negocio = ?,
           responsable_negocio = ?,
           ...
           motivo_rechazo = NULL,
           campo_rechazado = NULL
         WHERE usuario_id = ?`).run(ahora(), nombre, responsable, ubicacion.value.lat, ubicacion.value.lng, link, req.user.id);
      const expediente = await obtenerExpedienteVerificacion(db, req.user.id);
      await db.prepare('UPDATE verificaciones SET solicitud_json = ? WHERE usuario_id = ?').run(expediente ? JSON.stringify(expediente) : null, req.user.id);
    })();
    return res.status(201).json({
      estado: 'pendiente',
      verificado: false,
  ...
  router.post('/negocio/solicitar', requireAuth, exigirTipo('negocio'), async (req, res) => {
    const nombre = String(req.body.nombre_negocio ?? '').trim();
    const link = String(req.body.link_red_social ?? '').trim();
    const errorNombre = validarNombreNegocio(nombre);
    if (errorNombre) {
    ...
    const verificacion = await asegurarVerificacion(req.user.id, 'negocio');
    const datos = {
    ...
    return await completarSiPuedeCobrar(res, verificacion, {});
  });
  ...
  const externoNoVerificable = (_req, res) => res.status(403).json({
  ...
  router.get('/estado', requireAuth, async (req, res) => {
    ...
    const verificacion = await obtenerVerificacion(req.user.id);
    const tipoCuenta = seller.tipo_cuenta || 'particular';
    let puedeReintentar = 0;
    if (verificacion && verificacion.ventana_envio_inicio && verificacion.intentos_envio >= MAX_ENVIOS) {
      const restantes = minutosRestantes(verificacion.ventana_envio_inicio);
      const ventanaVigente = new Date(verificacion.ventana_envio_inicio).getTime() > Date.now() - VENTANA_ENVIO_MINUTOS * 60 * 1000;
    ...
    res.json({
      ...
      solicitud_manual_pendiente: Boolean(!seller.verified && verificacion?.tipo_cuenta === 'negocio' && verificacion?.estado === 'pendiente' && verificacion?.responsable_negocio),
      ...
      requisitos: await requisitosDeVerificacion(req.user.id),
 ...
 */
async function completarVerificacionPendientePorPagos(usuarioId) {
  ...
  const fila = await conexionDb.prepare('SELECT * FROM verificaciones WHERE usuario_id = ?').get(usuarioId);
  ...
  await conexionDb.transaction(async () => {
    ...
    await conexionDb.prepare(`UPDATE sellers SET verified = 1, carrera = @carrera, tipo_verificacion = @tipo
         WHERE id = @usuarioId`).run({
      usuarioId,
  ...
  sellers.length = 0;
  sellers.push(...(await db.getSellers()));
 ...
 * toca.
 */
async function reconciliarVerificacionesPendientesPorMercadoPago() {
  const db = require('../database');
  const filas = await db.getDb().prepare(`SELECT usuario_id FROM verificaciones
       WHERE estado != 'verificado' AND campo_rechazado = 'mercadopago'`).all();
  let resueltas = 0;
  for (const {
    usuario_id: usuarioId
  } of filas) {
    if (await completarVerificacionPendientePorPagos(usuarioId)) resueltas += 1;
  }
  if (resueltas > 0) {
...
function register(app) {
  ...
  const {
    verificarLink
  } = require('../services/linkCheck');
  app.use('/api/verificacion', crearRutasVerificacion({
    getDb: () => db.getDb(),
    mailer,
    sms,
    verificarLink,

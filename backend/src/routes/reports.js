...
function register(app) {
  app.post('/api/reports', requireAuth, (req, res) => {
    if (req.user.anon) {
      return res.status(403).json({ error: 'Inicia sesion para reportar.' });
    }
    ...
    if (!TARGET_TYPES.has(targetType) || !targetId || reason.length < 3) {
      return res.status(400).json({ error: 'Reporte invalido.' });
    }
    try {
      ...
      return res.status(400).json({ error: error.message || 'Reporte invalido.' });
    }
  });

  app.get('/api/me/reports', requireAuth, (req, res) => {
    if (req.user.anon) return res.json({ reports: [] });

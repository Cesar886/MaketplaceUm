...
function register(app) {
  ...
  app.get('/api/products/:id/comments', async (req, res) => {
    try {
      ...
      const {
        comments,
        nextCursor
      } = await db.getProductComments(req.params.id, {
      ...
      res.json({
        ...
        total: await db.countProductComments(req.params.id)
      });
    } catch (err) {
  ...
  app.post('/api/products/:id/comments', requireAuth, async (req, res) => {
    try {
      ...
      const autor = await db.getDb().prepare('SELECT id, name FROM sellers WHERE id = ?').get(autorId);
      ...
      const desdeUltimo = await db.segundosDesdeUltimoComentario(autorId);
      ...
      const comentario = await db.createProductComment(nuevoIdComentario(), producto.id, autorId, saneado.value);
      ...
      if (producto.seller && producto.seller !== autorId) {
        const preview = saneado.value.length > LARGO_PREVIEW ? `${saneado.value.slice(0, LARGO_PREVIEW)}…` : saneado.value;
        ...
        await db.createNotification(`notif_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`, producto.seller, 'product_comment', titulo, cuerpo, datos);
        await sendPush([producto.seller], titulo, cuerpo, datos);
      }
      res.status(201).json({
        comment: comentario,
        total: await db.countProductComments(producto.id)
      });
    } catch (err) {
      console.error('Error en POST /api/products/:id/comments:', err);
      res.status(500).json({
        error: 'No se pudo publicar el comentario. Intenta de nuevo.'
      });
    }
  });
  ...
  app.delete('/api/products/:id/comments/:commentId', requireAuth, async (req, res) => {
    try {
      const producto = products.find(p => p.id === req.params.id);
      if (!producto) return res.status(404).json({
        error: 'Producto no encontrado'
      });
      const comentario = await db.getProductCommentRow(req.params.commentId);
      ...
      const borrado = await db.softDeleteProductComment(comentario.id, actorId);
      if (!borrado) {
  ...
  app.get('/api/users/:id/comments', async (req, res) => {
    try {
      const existe = await db.getDb().prepare('SELECT 1 FROM sellers WHERE id = ?').get(req.params.id);
      ...
      const {
        comments,
        nextCursor
      } = await db.getCommentsReceivedBySeller(req.params.id, {
        limit: req.query.limit,
        cursor: req.query.cursor
      });
      res.json({
        comments,
        nextCursor,
        total: await db.countCommentsReceivedBySeller(req.params.id)
      });
    } catch (err) {

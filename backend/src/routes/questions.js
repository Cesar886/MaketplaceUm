...
const {
  ...
  sanitizarRespuesta,
  LARGO_MAXIMO
} = require('../validation/comentarios');
...
function register(app) {
  ...
  app.get('/api/products/:id/questions', async (req, res) => {
    try {
      ...
      const total = await db.countProductQuestions(producto.id);
      const pendingCount = await db.countPendingProductQuestions(producto.id);
      if (req.query.preview === 'true') {
        const limite = Math.min(parseInt(req.query.limit, 10) || db.PREGUNTAS_PREVIEW, db.PREGUNTAS_PREVIEW);
        return res.json({
          questions: await db.getProductQuestionsPreview(producto.id, {
            limit: limite
          }),
      ...
      const {
        questions,
        nextCursor
      } = await db.getProductQuestions(producto.id, {
  ...
  app.post('/api/products/:id/questions', requireAuth, async (req, res) => {
    try {
      ...
      if (saneado.error) return res.status(400).json({
        error: saneado.error
      });
      const desdeUltima = await db.segundosDesdeUltimaPregunta(autorId);
      ...
      const recientes = await db.contarPreguntasRecientes(autorId, producto.id, VENTANA_HORAS);
      ...
      const pregunta = await db.createProductQuestion(nuevoIdPregunta(), producto.id, producto.seller, autorId, saneado.value);
      ...
      if (producto.seller) {
        ...
        await db.createNotification(nuevoIdNotificacion(), producto.seller, 'product_question', titulo, cuerpo, datos);
        await sendPush([producto.seller], titulo, cuerpo, datos);
      }
      res.status(201).json({
        question: pregunta,
        total: await db.countProductQuestions(producto.id),
        pendingCount: await db.countPendingProductQuestions(producto.id)
      });
    } catch (err) {
      console.error('Error en POST /api/products/:id/questions:', err);
      res.status(500).json({
        error: 'No se pudo publicar la pregunta. Intenta de nuevo.'
      });
    }
  });
  ...
  app.delete('/api/products/:id/questions/:questionId', requireAuth, async (req, res) => {
    try {
      const producto = products.find(p => p.id === req.params.id);
      if (!producto) return res.status(404).json({
        error: 'Producto no encontrado'
      });
      const fila = await db.getProductQuestionRow(req.params.questionId);
      ...
      await db.deleteProductQuestion(fila.id);
      res.json({
        success: true,
        total: await db.countProductQuestions(producto.id),
        pendingCount: await db.countPendingProductQuestions(producto.id)
  ...
  app.post('/api/questions/:id/answer', requireAuth, async (req, res) => {
    try {
      const fila = await db.getProductQuestionRow(req.params.id);
      if (!fila) return res.status(404).json({
        error: 'Pregunta no encontrada'
      });
      ...
      const pregunta = await db.answerProductQuestion(fila.id, saneado.value);
      ...
      if (fila.asked_by && fila.asked_by !== req.user.id) {
        ...
        await db.createNotification(nuevoIdNotificacion(), fila.asked_by, 'question_answered', titulo, cuerpo, datos);
        await sendPush([fila.asked_by], titulo, cuerpo, datos);
      }
      res.json({
        question: pregunta,
        pendingCount: await db.countPendingProductQuestions(fila.product_id)
      });
    } catch (err) {
      console.error('Error en POST /api/questions/:id/answer:', err);
      res.status(500).json({
        error: 'No se pudo guardar la respuesta. Intenta de nuevo.'
      });
    }
  });
}

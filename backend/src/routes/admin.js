...
function loginLimiter({
  ...
  return rateLimit({
    ...
    message: {
      error: 'Demasiados intentos de acceso. Intenta de nuevo mas tarde.'
    }
  });
}
...
async function getConfig(database, key) {
  const row = await database.prepare(`${CONFIG_SELECT} WHERE key = ?`).get(key);
  return row ? rowToPublicationPolicy(row) : null;
}
function validateConfigPayload(body) {
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    return {
      error: 'El cuerpo debe ser un objeto de configuracion.'
    };
  }
  const receivedFields = Object.keys(body).sort();
  const expectedFields = [...PUBLICATION_POLICY_FIELDS].sort();
  if (receivedFields.length !== expectedFields.length || receivedFields.some((field, index) => field !== expectedFields[index])) {
    return {
  ...
  return {
    value: Object.fromEntries(PUBLICATION_POLICY_FIELDS.map(field => [field, body[field]]))
  };
}
async function writeConfig(database, key, policy, adminId, updatedAt) {
  await database.prepare(`UPDATE config SET products_active = ?, products_daily = ?,
       wanted_active = ?, wanted_daily = ?, duration_days = ?,
       updated_by_admin_id = ?, updated_at = ?
     WHERE key = ?`).run(policy.productsActive, policy.productsDaily, policy.wantedActive, policy.wantedDaily, policy.durationDays, adminId, updatedAt, key);
}
function router() {
  ...
  api.post('/auth/logout', async (req, res) => {
    await db.getDb().prepare(`INSERT OR IGNORE INTO admin_revoked_tokens
       (jti, admin_id, expires_at) VALUES (?, ?, ?)`).run(req.adminSession.jti, req.admin.id, req.adminSession.expiresAt);
    res.status(204).end();
  });
  api.get('/config', async (_req, res) => {
    const rows = await db.getDb().prepare(CONFIG_SELECT).all();
    const rowsByKey = new Map(rows.map(row => [row.key, row]));
    const config = PUBLICATION_POLICY_KEYS.map(key => rowsByKey.get(key)).filter(Boolean).map(rowToPublicationPolicy);
  ...
  api.get('/config/:key', async (req, res) => {
    if (!configKeyExists(req.params.key)) {
      return res.status(404).json({
        error: 'Configuracion no encontrada.'
      });
    }
    const config = await getConfig(db.getDb(), req.params.key);
  ...
  api.put('/config/:key', async (req, res) => {
    ...
    const result = await database.transaction(async () => {
      const before = await getConfig(database, req.params.key);
      if (!before) throw new Error('La configuracion de publicaciones esta incompleta.');
      await writeConfig(database, req.params.key, validated.value, req.admin.id, updatedAt);
      const after = await getConfig(database, req.params.key);
  ...
  api.delete('/config/:key', async (req, res) => {
    if (!configKeyExists(req.params.key)) {
      return res.status(404).json({
        error: 'Configuracion no encontrada.'
      });
    ...
    const result = await database.transaction(async () => {
      const before = await getConfig(database, req.params.key);
      if (!before) throw new Error('La configuracion de publicaciones esta incompleta.');
      await writeConfig(database, req.params.key, DEFAULT_PUBLICATION_POLICIES[req.params.key], req.admin.id, updatedAt);
      const after = await getConfig(database, req.params.key);
      await registrarAuditoriaAdmin(database, req, {
        action: 'config.reset',
        entityType: 'config',
  ...
  api.use('/revision', revision.router({

...
function register(app) {
  ...
  app.get('/api/feed', async (req, res) => {
    ...
    const ranked = await db.getFeedRanked({
      deviceId,
      userId,
      limit: limit + DIVERSITY_WINDOW,
      offset
    });
    ...
    const enriched = await attachRelations(diversified, userId || deviceId);
    res.json({
      deviceId,
      userId,
  ...
  app.post('/api/interacciones', optionalAuth, async (req, res) => {
    const {
      deviceId,
      productId,
      categoryId,
    ...
    if (!deviceId || !tipo) {
      return res.status(400).json({
        error: 'deviceId y tipo son obligatorios'
      });
    }
    ...
    if (tipo === 'categoria') {
      if (!categoryId) {
        return res.status(400).json({
          error: "categoryId es obligatorio para tipo 'categoria'"
        });
      }
      const categoria = await db.getDb().prepare('SELECT id FROM categories WHERE id = ?').get(categoryId);
      if (!categoria) {

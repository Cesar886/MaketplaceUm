...
const DUMMY_PASSWORD_HASH = bcrypt.hashSync('credencial-administrativa-inexistente', 12);
function configuredSecret(name) {
  const value = String(process.env[name] || '');
  return value.length >= 32 ? value : null;
}
function adminAuthConfigured() {
  return !!configuredSecret('ADMIN_JWT_SECRET') && !!configuredSecret('ADMIN_TOTP_ENCRYPTION_KEY');
}
...
function encryptTotpSecret(base32Secret) {
  ...
  const encrypted = Buffer.concat([cipher.update(base32Secret.toUpperCase(), 'utf8'), cipher.final()]);
...
function decryptTotpSecret(value) {
  ...
  const decipher = crypto.createDecipheriv('aes-256-gcm', encryptionKey(), Buffer.from(match[1], 'base64url'));
...
async function requireAdmin(req, res, next) {
  ...
  if (!token) return res.status(401).json({
    error: 'Sesion administrativa requerida.'
  });
  try {
    ...
    if (payload.typ !== 'admin' || typeof payload.sub !== 'string' || typeof payload.jti !== 'string' || !Number.isSafeInteger(payload.exp) || !Number.isSafeInteger(payload.ver)) {
      return res.status(401).json({
        error: 'Sesion administrativa invalida.'
      });
    }
    const admin = await db.getDb().prepare(`SELECT id, username, token_version FROM admins
       WHERE id = ? AND active = 1`).get(payload.sub);
    if (!admin || admin.token_version !== payload.ver) {
      return res.status(401).json({
        error: 'Sesion administrativa invalida.'
      });
    }
    const revoked = await db.getDb().prepare('SELECT 1 FROM admin_revoked_tokens WHERE jti = ? AND expires_at > unixepoch()').get(payload.jti);
    ...
    return res.status(401).json({
      error: 'Sesion administrativa invalida o expirada.'
    });
  }
}
async function authenticateAdmin({
  ...
  const safePassword = typeof password === 'string' && password.length <= 512 ? password : '';
  const safeTotp = typeof totp === 'string' && /^\d{6}$/.test(totp.trim()) ? totp.trim() : '';
  const admin = normalizedUsername ? await db.getDb().prepare(`SELECT id, username, password_hash, totp_secret_encrypted,
       token_version, last_totp_step
       FROM admins WHERE username = ? AND active = 1`).get(normalizedUsername) : null;
  const passwordMatches = await bcrypt.compare(safePassword, admin?.password_hash || DUMMY_PASSWORD_HASH);
  if (!admin || !passwordMatches || !safeTotp) return {
    authenticated: false
  };
  let secret;
  try {
  ...
  const claimed = await db.getDb().prepare(`UPDATE admins SET last_totp_step = ?, last_login_at = datetime('now'),
       updated_at = datetime('now')
     WHERE id = ? AND (last_totp_step IS NULL OR last_totp_step < ?)`).run(totpStep, admin.id, totpStep);

...
function textoAuditable(value, field, maxLength, pattern = null) {
  if (typeof value !== 'string' || value.length === 0 || value.length > maxLength || /[\u0000-\u001f\u007f]/.test(value) || pattern && !pattern.test(value)) {
...
function serializarDetallesAuditoria(details = {}) {
  ...
  if (Buffer.byteLength(serialized, 'utf8') > MAX_DETAILS_JSON_BYTES) {
    throw new RangeError(`Los detalles de auditoria no pueden superar ${MAX_DETAILS_JSON_BYTES} bytes.`);
  }
  return serialized;
}
 ...
 */
async function registrarAuditoriaAdmin(database, req, {
  action,
  entityType,
  entityId,
  details = {},
  ...
  if (!admin || !Number.isSafeInteger(admin.id) || admin.id <= 0 || typeof admin.username !== 'string' || admin.username.length < 3 || admin.username.length > 64 || /[\u0000-\u001f\u007f]/.test(admin.username)) {
    throw new Error('Se requiere un administrador autenticado para registrar la auditoria.');
  }
  const safeAction = textoAuditable(action, 'action', MAX_ACTION_LENGTH, AUDIT_KEY_PATTERN);
  const safeEntityType = textoAuditable(entityType, 'entityType', MAX_ENTITY_TYPE_LENGTH, AUDIT_KEY_PATTERN);
  const safeEntityId = textoAuditable(String(entityId), 'entityId', MAX_ENTITY_ID_LENGTH);
  const safeCreatedAt = textoAuditable(createdAt, 'createdAt', 40);
  const detailsJson = serializarDetallesAuditoria(details);
  return await database.prepare(`INSERT INTO admin_audit_log (
       admin_id, action, entity_type, entity_id, details_json, created_at
     ) VALUES (?, ?, ?, ?, ?, ?)`).run(admin.id, safeAction, safeEntityType, safeEntityId, detailsJson, safeCreatedAt);
}
module.exports = {

...
if (!JWT_SECRET) {
  throw new Error('JWT_SECRET no está definido en el entorno. Revisa que backend/.env exista ' + 'y que PM2 lo esté cargando (pm2 restart mercadito-backend --update-env).');
}
...
async function sesionRevocada(decoded) {
...
async function generateRefreshToken(userId) {
  ...
  await db.getDb().prepare(`INSERT INTO refresh_sessions (token_hash, user_id)
     VALUES (?, ?)`).run(hashRefreshToken(refreshToken), userId);
  return refreshToken;
}
...
async function refreshSession(refreshToken) {
  ...
  const row = await database.prepare(`SELECT rs.user_id
       FROM refresh_sessions rs
       JOIN sellers s ON s.id = rs.user_id
      WHERE rs.token_hash = ? AND rs.revoked_at IS NULL`).get(tokenHash);
  if (!row) return null;
  ...
  const access = await getSellerAccess(database, row.user_id);
  if (!access.allowed) return null;
  await database.prepare(`UPDATE refresh_sessions SET last_used_at = datetime('now')
      WHERE token_hash = ?`).run(tokenHash);
  return {
    token: generateToken(row.user_id),
    userId: row.user_id
  };
}
async function revokeRefreshToken(refreshToken, expectedUserId = null) {
  ...
  await db.getDb().prepare(sql).run(...params);
}
...
async function requireAuth(req, res, next) {
  ...
  if (await sesionRevocada(decoded)) {
    return res.status(401).json({
      error: 'SESSION_INVALIDATED',
  ...
  if (isAnonymous) {
    ...
    let access;
    try {
      access = await getSellerTokenAccess(db.getDb(), decoded);
    } catch (error) {
...
async function optionalAuth(req, _res, next) {
  ...
  try {
    ...
    if (await sesionRevocada(decoded)) return next();
    ...
    const access = await getSellerTokenAccess(db.getDb(), decoded);
    if (access.allowed) req.user = {
      id: decoded.sub,
      anon: false
    };
  } catch (err) {
...
async function verificarToken(token) {
  ...
  try {
    ...
    if (await sesionRevocada(decoded)) return null;
    if (decoded.anon === true) {
      return typeof decoded.sub === 'string' && /^anon_[0-9a-f-]{36}$/i.test(decoded.sub) ? decoded.sub : null;
    }
    const access = await getSellerTokenAccess(db.getDb(), decoded);
    if (!access.allowed) return null;

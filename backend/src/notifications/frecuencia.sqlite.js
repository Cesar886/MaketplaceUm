 ...
 */
function estaHabilitado(subjectId, tipo = TIPO_RETARGETING) {
  const fila = db.getDb().prepare(`
...
function setHabilitado(subjectId, tipo, habilitado) {
  db.getDb().prepare(`
 ...
 */
function ignoradasSeguidas(subjectId, categoryId = null) {
  const filas = db.getDb().prepare(`
    ...
    LIMIT ?
  `).all(
    subjectId, TIPO_RETARGETING, categoryId, categoryId,
    GRACIA_APERTURA_HORAS, IGNORADAS_PARA_CASTIGO
  );
 ...
 */
function puedeEnviar(subjectId, categoryId) {
  if (!estaHabilitado(subjectId)) {
    return { permitido: false, motivo: 'preferencia_desactivada' };
  }
  ...
  if (ignoradasSeguidas(subjectId, categoryId) >= IGNORADAS_PARA_CASTIGO) {
    const enPausa = raw.prepare(`
    ...
    if (enPausa) {
      return { permitido: false, motivo: 'categoria_pausada' };
    }
  }
  ...
  if (total >= MAX_POR_SEMANA) {
    return { permitido: false, motivo: 'tope_semanal' };
  }
  ...
  const intervaloHoras = ignoradasSeguidas(subjectId, null) >= IGNORADAS_PARA_CASTIGO
    ? MIN_HORAS_MISMA_CATEGORIA * MULTIPLICADOR_INTERVALO
    : MIN_HORAS_MISMA_CATEGORIA;

  const reciente = raw.prepare(`
    SELECT 1 FROM notification_log
    WHERE subject_id = ? AND type = ? AND category_id = ?
      AND sent_at >= datetime('now', '-' || ? || ' hours')
    LIMIT 1
  ...
  if (reciente) {
    return { permitido: false, motivo: 'intervalo_categoria', intervaloHoras };
  }

  return { permitido: true, motivo: 'ok', intervaloHoras };
}
 ...
 */
function registrarEnvio({ subjectId, categoryId, productIds, notificationId }) {
 ...
 */
function registrarApertura({ subjectId, notificationId, categoryId }) {
  const raw = db.getDb();

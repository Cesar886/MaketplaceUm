 ...
 */
function productosSinStock(usuarioId) {
  return db.getDb()
    .prepare(
      `SELECT id, title, manual_status FROM products
       WHERE seller = ? AND stock_quantity IS NULL
       ORDER BY title`,
    )
    .all(usuarioId)
    .filter(p => !ESTADOS_FUERA_DE_VENTA.has(p.manual_status))
...
function requisitosDeVerificacion(usuarioId) {
  ...
  return [
    ...
    },
    {
      ...
      detalle: sinStock.length === 0
        ? 'Todos tus productos tienen cantidad definida.'
...
function cumpleTodos(usuarioId) {
  return requisitosDeVerificacion(usuarioId).every(r => r.cumplido);
}
 ...
 * no se lee.
 */
function primerFaltante(usuarioId) {
  return requisitosDeVerificacion(usuarioId).find(r => !r.cumplido) || null;
}

...
test('dentro del horario está abierto', async () => {
  const v = crearVendedor({ '2': { open: '09:00', close: '18:00' } });
  assert.strictEqual((await estadoDeAtencion(v, miercoles(13))).abierto, true);
});

test('antes de abrir está cerrado y dice a qué hora abre', async () => {
  const v = crearVendedor({ '2': { open: '09:00', close: '18:00' } });
  const e = await estadoDeAtencion(v, miercoles(7, 30));
...
test('después de cerrar apunta al siguiente día con horario', async () => {
  const v = crearVendedor({
    '2': { open: '09:00', close: '18:00' },
    '4': { open: '10:00', close: '14:00' },
  });
  const e = await estadoDeAtencion(v, miercoles(20));
...
test('la hora de cierre ya está cerrado', async () => {
  // El rango es [open, close): a las 18:00 en punto ya no se atiende.
  const v = crearVendedor({ '2': { open: '09:00', close: '18:00' } });
  assert.strictEqual((await estadoDeAtencion(v, miercoles(18))).abierto, false);
});

test('un día sin horario busca el siguiente, dando la vuelta a la semana', async () => {
  const v = crearVendedor({ '0': { open: '08:00', close: '12:00' } });
  const e = await estadoDeAtencion(v, miercoles(10));

  assert.strictEqual(e.abierto, false);
  assert.strictEqual(e.diaAbre, 0, 'el próximo lunes');
});

test('sin horario configurado se considera abierto, no cerrado', async () => {
  ...
  const e = await estadoDeAtencion(v, miercoles(3));

  assert.strictEqual(e.abierto, true);
  assert.strictEqual(e.tieneHorario, false);
});

test('un horario corrupto no revienta ni bloquea', async () => {
  const id = `u_hor_${++n}`;
  db.getDb().prepare(
    `INSERT INTO sellers (id, name, email, avatarInitials, major, verified, businessHours)
  ...
  assert.strictEqual((await estadoDeAtencion(id, miercoles(10))).abierto, true);
});

test('un vendedor que no existe no bloquea el cobro', async () => {
  assert.strictEqual((await estadoDeAtencion('u_fantasma', miercoles(10))).abierto, true);
});

test('el mensaje dice cuándo vuelve a abrir', async () => {
...
test('sin nombre de vendedor el mensaje sigue leyéndose', async () => {
  const v = crearVendedor({ '2': { open: '09:00', close: '18:00' } });
  const msg = mensajeCerrado(null, await estadoDeAtencion(v, miercoles(7)));

 ...
 */
function estadoDeAtencion(sellerId, ahora = new Date()) {
  const fila = db.getDb()
    .prepare('SELECT businessHours FROM sellers WHERE id = ?')
    .get(sellerId);

  const horario = parsearHorario(fila?.businessHours);
  if (!horario) {
    return { abierto: true, tieneHorario: false, abreA: null, diaAbre: null };
  }
  ...
  if (deHoy && minutosAhora >= aMinutos(deHoy.open) && minutosAhora < aMinutos(deHoy.close)) {
    return { abierto: true, tieneHorario: true, abreA: null, diaAbre: null };
  }
  ...
  if (deHoy && minutosAhora < aMinutos(deHoy.open)) {
    return { abierto: false, tieneHorario: true, abreA: deHoy.open, diaAbre: hoy };
  }
  for (let i = 1; i <= 7; i++) {
    const dia = (hoy + i) % 7;
    const rango = horario[String(dia)];
    if (rango) {
      return { abierto: false, tieneHorario: true, abreA: rango.open, diaAbre: dia };
    }
  }

  return { abierto: false, tieneHorario: true, abreA: null, diaAbre: null };
}
...
function mensajeCerrado(nombreVendedor, estado) {
  ...
  const cuando = estado.diaAbre !== null
    ? ` el ${DIAS[estado.diaAbre]} a las ${estado.abreA}`
    : ` a las ${estado.abreA}`;
  return `${quien} está cerrado en este momento. Abre${cuando}.`;
}

module.exports = { estadoDeAtencion, mensajeCerrado, DIAS };
